...
```bash
npx -y github:larrywood0602/kaigongba-agent-connect-skill --api-base-url https://api.kaigongba.com --connect-code kgbc_xxx
```
...
Other scripts read this file automatically. Environment variables are only for local development overrides. If the skill is already installed, `scripts/bootstrap_connection.mjs` can still be run directly.
